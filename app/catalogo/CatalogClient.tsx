...
export default function CatalogClient() {
  ...
  const handleCreateCatalog = async () => {
    ...
    if (selectedCategories.length === 0) { alert("Selecione ao menos uma categoria."); return; }
  ...
  const handleSaveCatalog = async () => {
    ...
    try {
      ...
      const items = selectedCategories.map((category_id) => ({
          ...
          <div className="border border-gray-200 rounded-xl p-4">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2">
              {filteredCategories.map((cat) => (
                ...
                >
                  <div
                    className="w-8 h-8 rounded-lg flex-shrink-0 bg-cover bg-center bg-gray-200"
              ...
              {catalogs.map((catalog) => (
                <li
                  ...
                  className={`flex items-center justify-between rounded-lg px-3 py-2 cursor-pointer transition-colors ${
                    activeCatalogId === catalog.id ? "bg-gray-200" : "hover:bg-gray-100"
                  }`}
                  onClick={() => setActiveCatalogId(catalog.id)}
                >
                  <span className="text-sm text-gray-800 truncate">{catalog.name}</span>
                  <button
                    onClick={(e) => { e.stopPropagation(); handleDeleteCatalog(catalog.id); }}
          ...
          <div className="bg-white rounded-2xl p-6 w-full max-w-sm shadow-xl">
            ...
            <div className="flex gap-3">
              ...
              <button
                ...
                disabled={saving || !catalogName.trim()}
