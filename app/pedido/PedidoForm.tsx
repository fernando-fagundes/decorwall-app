...
export default function PedidoForm({ userId }: Props) {
  ...
  useEffect(() => {
    getSupabase().from("products").select("id, name, preco_m2, largura_min, largura_max, altura_min, altura_max").order("display_order").then(({ data }) => {
      if (data && data.length > 0) { setProdutos(data); setParedes([newParede(1, data[0])]); }
      else { setParedes([newParede(1)]); }
    });
  }, []);
  ...
  function removeParede(id: string) {
    setParedes(prev => prev.filter(p => p.id !== id));
    setImagensPorParede(prev => { const next = { ...prev }; delete next[id]; return next; });
  }
  ...
  function setImagemParede(paredeId: string, file: File | null) { setImagensPorParede(prev => ({ ...prev, [paredeId]: file })); }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    // Upload layout images to Supabase Storage
    const layoutUrls: Record<string, string> = {};
    for (const [paredeId, file] of Object.entries(imagensPorParede)) {
      ...
      if (!uploadError) {
        ...
        layoutUrls[paredeId] = urlData.publicUrl;
      }
    }
    ...
    const itens = paredes.map(p => ({
      ...
      preco_m2: p.preco_m2,
      largura_cm: n(p.largura),
      altura_cm: n(p.altura),
      ...
      layout_url: layoutUrls[p.id] || null,
    }));
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-8">
      ...
      <section className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <input type="text" value={cidade} onChange={e => setCidade(e.target.value)} className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-gray-900" />
          </div>
          <div>
            <label className="block text-sm text-gray-600 mb-1">Estado</label>
            <input type="text" value={estado} onChange={e => setEstado(e.target.value)} maxLength={2} className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm uppercase focus:outline-none focus:ring-2 focus:ring-gray-900" />
          </div>
        </div>
      </section>
      ...
      <section className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
        ...
        <div className="space-y-6">
          {paredes.map((parede, idx) => {
            ...
            const imagem = imagensPorParede[parede.id] ?? null;
            return (
                ...
                <div className="p-4 space-y-4">
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                    <div>
                      ...
                      <input type="number" min="0" step="1" value={parede.largura} onChange={e => updateParede(parede.id, { largura: e.target.value })} placeholder="ex: 350" className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-gray-900" />
                    </div>
                    <div>
                      <label className="block text-xs text-gray-500 mb-1">Altura (cm)</label>
                      <input type="number" min="0" step="1" value={parede.altura} onChange={e => updateParede(parede.id, { altura: e.target.value })} placeholder="ex: 270" className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-gray-900" />
                    </div>
                  ...
                  {/* Upload de imagem do layout aprovado */}
                  <div>
                    <label className="block text-xs text-gray-500 mb-2">Layout aprovado</label>
      ...
      {/* 3. Transportadora */}
      <section className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
        <h2 className="text-base font-semibold text-gray-800 mb-4">3. Transportadora</h2>
        <input
          type="text"
